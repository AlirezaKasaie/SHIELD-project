...
# combined_dataset$disch_date_time <- sub("^\\[|\\]$", "", combined_dataset$disch_date_time)
# combined_dataset$disch_date_time <- as.POSIXct(combined_dataset$disch_date_time, format="%Y/%m/%d:%I:%M:%S %p")

combined_dataset$last_icu_dt <- sub("^\\[|\\]$", "", combined_dataset$last_icu_dt)
combined_dataset$last_icu_dt <- as.POSIXct(combined_dataset$last_icu_dt, format="%Y/%m/%d:%I:%M:%S %p")

# combined_dataset$death_date <- sub("^\\[|\\]$", "", combined_dataset$death_date)
# combined_dataset$death_date <- as.POSIXct(combined_dataset$death_date, format="%Y/%m/%d:%I:%M:%S %p")
...
# combined_dataset = combined_dataset %>%
#   rename(Hospital_Discharge_Date = disch_date_time)

combined_dataset$Date = format(combined_dataset$Date, "%Y-%m-%d")

# combined_dataset$Death_Date = format(combined_dataset$Death_Date, "%Y-%m-%d")
# 
# combined_dataset$Hospital_Discharge_Date = format(combined_dataset$Hospital_Discharge_Date, "%Y-%m-%d")
...
combined_dataset <- combined_dataset %>%
  ...
  filter(st == "IL", 
         year(Date) %in% c(2020, 2021, 2022,2023), 
         ref_bill_code %in% covid_codes) %>%
  mutate(Date = format(Date, "%Y-%m-%d"))

unique_zipcodes_ICU <- n_distinct(combined_dataset$zipcode)
...
combined_dataset = combined_dataset[,-c(2,4:5,9:10,12,14,16,18,21:45)]
combined_dataset = combined_dataset[,-c(8)]

# combined_dataset = combined_dataset[,-c(6,8:9)]
# 
# selected_columns <- combined_dataset[, c(1,5,7)]


unique_zipcodes <- unique(combined_dataset$zipcode)

# Get the total number of unique zip codes
# total_unique_zipcodes <- length(unique_zipcodes)
# 
# 
# patients_per_zipcode_date <- selected_columns %>%
#   group_by(zipcode, Date) %>%
#   summarise(total_patients = n_distinct(mrn))
# 
# 
# filtered_zipcodes <- patients_per_zipcode %>%
#   filter(total_patients >= 10)
# 
# unique_zipcodes <- unique(filtered_zipcodes$zipcode)
...
patients_per_zipcode <- filtered_combined_dataset %>%
  group_by(zipcode) %>%
...
filtered_zipcodes = filtered_zipcodes %>%
  rename(hsp_account_id = total_patients)

New_Dataset <- merge(filtered_combined_dataset, filtered_zipcodes, by = c("zipcode"))

unique_zipcodes <- unique(New_Dataset$zipcode)

New_Dataset = New_Dataset[,-c(3)]

New_Dataset$Date <- ymd(New_Dataset$Date)

New_Dataset$Date <- format(New_Dataset$Date, "%Y-%m")

new_column <- New_Dataset %>%
  group_by(zipcode, Date) %>%
  summarise(Total_ICU_admissions_per_zipcode_per_month = n())

New_Dataset_UPDATED <- New_Dataset %>%
  left_join(new_column, by = c("zipcode", "Date"))

unique_zipcodes <- unique(New_Dataset_UPDATED$zipcode)

New_Dataset_UPDATED = New_Dataset_UPDATED[,-c(10)]

New_Dataset_UPDATED = New_Dataset_UPDATED[,-c(2:6,8:9)]

New_Dataset_UPDATED <- New_Dataset_UPDATED %>%
  distinct()

write.csv(New_Dataset_UPDATED, file = "R:/mtootooni/217107/Shield Data/Alireza/Shield/Data/ICU/Method 1/ICU_UPDATED.csv", row.names = FALSE)

# write.csv(filtered_zipcodes, file = "R:/mtootooni/217107/Shield Data/Alireza/Shield/Data/ICU/Method 4/filtered_zipcodes.csv", row.names = FALSE)
# 
# write.csv(combined_dataset, file = "R:/mtootooni/217107/Shield Data/Alireza/Shield/Data/ICU/Method 4/ICU_DATASET.csv", row.names = FALSE)
# 
# filtered_zipcodes_NEW = fread(file.choose(), sep=",", header=T)
# 
# filtered_zipcodes_NEW$zipcode = as.character(filtered_zipcodes_NEW$zipcode)
# 
# combined_dataset$zipcode = as.character(combined_dataset$zipcode)
# 
# ICU_UPDATED <- merge(filtered_zipcodes_NEW, combined_dataset, by = c("zipcode"))
# 
# write.csv(ICU_UPDATED, file = "R:/mtootooni/217107/Shield Data/Alireza/Shield/Data/ICU/Method 4/ICU_UPDATED.csv", row.names = FALSE)
...
# IL_zipcodes = unique(combined_dataset$zipcode)
# 
# 
...
#   filter(ZCTA5CE10 %in% combined_dataset$zipcode )
# 
...
# library(geojsonio)
# 
# url <- "https://raw.githubusercontent.com/johan/world.geo.json/master/countries/USA/IL.geo.json"
# 
# illinois_boundary <- st_read(url)
# 
# leaflet() %>%
...
#   addPolygons(data = illinois_boundary, fillColor = "blue", fillOpacity = 0.1, color = "blue", weight = 2, smoothFactor = 0.5)
