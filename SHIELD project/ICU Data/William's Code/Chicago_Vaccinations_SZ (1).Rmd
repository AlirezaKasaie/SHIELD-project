...
vax_by_zip <- vax_by_zip %>%
  ...
  dplyr:: select(zip, week_st, cumulative_doses, cumulative_first, cumulative_complete, cumulative_pct_first, cumulative_pct_complete, doses_weekly, first_weekly, `1st Dose - Cumulative`) %>%
...
alphavaxmap <- left_join(alpha_groups, zips_sf) %>%
  ggplot(aes(geometry = geometry)) + 
    ...
    vaxmapstyle +
  labs(fill = paste0("% of Population with First Doses\n(", alpha_grouping_date_chr, ")"))
...
quantile(economics_social_merged$`Median Age (Years)`)
quantile(economics_social_merged$`Median Household Income ($)`)
```
...
low <- economics_social_by_zip %>%
  filter(vax_group_alpha == n_labels_alpha[1]) %>%
  mutate(`Median Household Income ($)` = as.numeric (`Median Household Income ($)`))

quantile(low$`Median Age (Years)`)
quantile(low$`Median Household Income ($)`)

mid <- economics_social_by_zip %>%
  filter(vax_group_alpha == n_labels_alpha[2]) %>%
  mutate(`Median Household Income ($)` = as.numeric (`Median Household Income ($)`))

quantile(mid$`Median Age (Years)`)
quantile(mid$`Median Household Income ($)`)

high <- economics_social_by_zip %>%
  filter(vax_group_alpha == n_labels_alpha[3]) %>%
  mutate(`Median Household Income ($)` = as.numeric (`Median Household Income ($)`))

quantile(high$`Median Age (Years)`)
quantile(high$`Median Household Income ($)`)
...
vaxlevel_zip_alpha_2weeks <- joined_df %>%
  filter(week_st == alpha_grouping_date+28 & !zip %in% outliers$zip) %>% 
  # deciles 
  mutate(alpha_vaxleveldec2weeks = cumulative_pct_first / 10,
         alpha_completeleveldec2weeks = cumulative_pct_complete / 10,
         alpha_recoveredleveldec2weeks = (cases_cumulative - deaths_cumulative)/population * 10) %>%
  dplyr::select(zip, population, alpha_vaxleveldec2weeks, alpha_recoveredleveldec2weeks, alpha_completeleveldec2weeks)

vaxlevel_zip_alpha_8weeks <- joined_df %>%
  filter(week_st == alpha_grouping_date-14 & !zip %in% outliers$zip) %>% 
  # deciles 
  mutate(alpha_vaxleveldec8weeks = cumulative_pct_first / 10,
         alpha_completeleveldec8weeks = cumulative_pct_complete / 10,
         alpha_recoveredleveldec8weeks = (cases_cumulative - deaths_cumulative)/population * 10) %>%
  dplyr::select(zip, population, alpha_vaxleveldec8weeks, alpha_recoveredleveldec8weeks, alpha_completeleveldec8weeks)
...
mixedeffectsdf_alpha_zip <- joined_df %>%
  ...
  left_join(vaxlevel_zip_alpha_2weeks) %>%
  left_join(vaxlevel_zip_alpha_8weeks) %>%
...
vaxlevel_zip_delta_2weeks <- joined_df %>%
  filter(week_st == delta_grouping_date+28 & !zip %in% outliers$zip) %>% 
  # deciles 
  mutate(delta_vaxleveldec2weeks = cumulative_pct_first / 10,
         delta_completeleveldec2weeks = cumulative_pct_complete / 10,
         delta_recoveredleveldec2weeks = (cases_cumulative - deaths_cumulative)/population * 10) %>%
  dplyr::select(zip, population, delta_vaxleveldec2weeks, delta_recoveredleveldec2weeks, delta_completeleveldec2weeks)

vaxlevel_zip_delta_8weeks <- joined_df %>%
  filter(week_st == delta_grouping_date-14 & !zip %in% outliers$zip) %>% 
  # deciles 
  mutate(delta_vaxleveldec8weeks = cumulative_pct_first / 10,
         delta_completeleveldec8weeks = cumulative_pct_complete / 10,
         delta_recoveredleveldec8weeks = (cases_cumulative - deaths_cumulative)/population * 10) %>%
  dplyr::select(zip, population, delta_vaxleveldec8weeks, delta_recoveredleveldec8weeks, delta_completeleveldec8weeks)
...
mixedeffectsdf_delta_zip <- joined_df %>%
  ...
  left_join(vaxlevel_zip_delta_2weeks) %>%
  left_join(vaxlevel_zip_delta_8weeks) %>%
  left_join(age_by_zip) %>%
  dplyr::group_by(wave) %>%
  mutate(wave_week = week - min(week)) %>%
  dplyr::group_by(wave, zip) %>%
...
### Model 1 (Test for parallel trends)
...
## Model 2 Mixed Effects (unadjusted)
...
tab_model(model2_alpha_zip, model3_alpha_zip, transform = NULL, show.se = TRUE)
...
# upper bound
exp((2.41+1.96*0.28) + fixef(model2_alpha_zip)[4]*mean(vaxlevel_zip_alpha$alpha_vaxleveldec))
...
# upper bound
exp((2.39+1.96*0.27) + fixef(model3_alpha_zip)[6]*mean(vaxlevel_zip_alpha$alpha_vaxleveldec))
...
### Sensitivity analysis: Model 3b (with vax groups)
...
### Sensitivity analysis - Model 4: Levels of fully vaccinated
...
### Sensitivity Analysis - Model 3: Lag Time

```{r}

# alpha Zip

model3_alpha_zip_2weeks <- glmer(data = mixedeffectsdf_alpha_zip, deaths_weekly ~ (1|zip) + week + wave + wave:alpha_vaxleveldec2weeks + alpha_recoveredleveldec2weeks + dec65plus + offset(log(population)), family = poisson(link = "log"))

model3_alpha_zip_8weeks <- glmer(data = mixedeffectsdf_alpha_zip, deaths_weekly ~ (1|zip) + week + wave + wave:alpha_vaxleveldec8weeks + alpha_recoveredleveldec8weeks + dec65plus + offset(log(population)), family = poisson(link = "log"))

# Delta Zip

model3_delta_zip_2weeks <- glmer(data = mixedeffectsdf_delta_zip, deaths_weekly ~ (1|zip) + week + wave + wave:delta_vaxleveldec2weeks + delta_recoveredleveldec2weeks + dec65plus + offset(log(population)), family = poisson(link = "log"))

model3_delta_zip_8weeks <- glmer(data = mixedeffectsdf_delta_zip, deaths_weekly ~ (1|zip) + week + wave + wave:delta_vaxleveldec8weeks + delta_recoveredleveldec8weeks + dec65plus + offset(log(population)), family = poisson(link = "log"))

# 2 weeks before peak

model3_alpha_zip_2weeks

model3_delta_zip_2weeks

tab_model(model3_alpha_zip_2weeks, model3_delta_zip_2weeks)

# 8 weeks before peak

model3_alpha_zip_8weeks

model3_delta_zip_8weeks

tab_model(model3_alpha_zip_8weeks, model3_delta_zip_8weeks)
