...
TCOVID_NEW = TCOVID_NEW %>%
  filter(zipcode %in% ICU_UPDATED_FINAL_2$zipcode)
...
ICU_UPDATED_FINAL_2 = merge(ICU_UPDATED_FINAL_2, TCOVID_NEW_aggregated, by.x = c("zipcode", "Date"), by.y = c("zipcode", "DATE - COLLECTED"), all.x = TRUE)

ICU_UPDATED_FINAL_2 = ICU_UPDATED_FINAL_2%>%
  mutate(`Total_SHIELD_centers_per_zipcode_per_month` = replace_na(`Total_SHIELD_centers_per_zipcode_per_month`, 0))

write.csv(ICU_UPDATED_FINAL_2, file = "C:/Users/skasaiesharifi/Documents/ICU_UPDATED_FINAL_2.csv", row.names = FALSE)
